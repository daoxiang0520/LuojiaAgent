...
def query_user_reservations(
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            eval_js = eval_js.replace("{library_token}", creds := captured_credentials["token"])
            eval_js = eval_js.replace("{library_hmac}", captured_credentials["hmac"])
            eval_js = eval_js.replace("{library_request_date}", captured_credentials["xdate"])
            eval_js = f"async () => {{ const response = await fetch('{api_path}', {{ method: 'POST', headers: {{ 'Content-Type': 'application/json', 'token': '{library_token}', 'X-hmac-request-key': '{library_hmac}', 'X-request-date': '{library_request_date}', 'X-request-id': '{library_request_id}', 'loginType': 'PC' }}, body: JSON.stringify({{ 'currentPage': 1, 'pageSize': 15 }}) }}); return await response.json(); }}"
